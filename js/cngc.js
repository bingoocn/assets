...
! function($) {
  ...
  /**
   * 生成pickerTree的外侧picker元素.
   * @param  {json} setting pickerTree设置对象.
   * @return {undefined}
   */
  ...
  cngc.pickerTree = cngc.createUI(function(options) {
    ...
    init: function(opts) {
      ...
      var initValue = (cngc.utils.isEmptyValue(opts.selected) ? $('#' + opts.id).val() : opts.selected);
      if(!cngc.utils.isEmptyValue(initValue)) {
        me.setValue.call($.extend(true, {}, me, treeObj), initValue);
      }
      return treeObj;
    },
    hide: function() {
  ...
  cngc.select = function(options) {
    if (options.url !== undefined) {
      ...
      $.ajax({
        ...
        if(!cngc.utils.isEmptyValue(options.selected)) {
